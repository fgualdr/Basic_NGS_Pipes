...
#- shortstuck mapping
$Path_to_ShortStack/ShortStack --mismatches 2 --bowtie_m 100 --outdir $scratch/$tmp/1_ShortStack_align/$filename --nohp --bowtie_cores 4 --sort_mem 4G --align_only --mmap u --readfile $FILE --genomefile $BOWTIE_INDEX/genome.fa
gzip $FILE
...
echo "Sort"
samtools sort -m 4G -@ 4 $scratch/$tmp/2_BAM/"$filename".mapped.bam > $scratch/$tmp/2_BAM/"$filename".mapped.sort.bam
...
samtools sort -m 4G -@ 4 -n -o $scratch/$tmp/2_BAM/"$filename".namesort.bam $scratch/$tmp/2_BAM/"$filename".mapped.sort.clean.bam
# Add ms and MC tags for markdup to use later
samtools fixmate -m $scratch/$tmp/2_BAM/"$filename".namesort.bam $scratch/$tmp/2_BAM/"$filename".fixmate.bam
# Markdup needs position order
samtools sort -m 4G -@ 4 -o $scratch/$tmp/2_BAM/"$filename".positionsort.bam $scratch/$tmp/2_BAM/"$filename".fixmate.bam
...
if [ "$CALL_PEAKS" = "YES" ]; then
    ...
    if [ "$INPUT_CHIP" = "NA" ]; then
        echo NO INPUT
        if [ "$ORG" = "mm" ]; then
            macs2 callpeak -t $OUT_FOLDER/2_BAM/"$filename".clean.bam --name="$filename".filterdup.nol  --outdir $OUT_FOLDER/4_Called_Peaks/ --format=BAM -B -g mm --nomodel --extsize 250 --nolambda --broad --broad-cutoff 0.1 -m 250 500
        else
            if [ "$ORG" = "hg" ]; then
                macs2 callpeak -t $OUT_FOLDER/2_BAM/"$filename".clean.bam --name="$filename".filterdup.nol  --outdir $OUT_FOLDER/4_Called_Peaks/ --format=BAM -B -g hs --nomodel --extsize 250 --nolambda --broad --broad-cutoff 0.1 -m 250 500
            fi
        fi
    else
        if [ "$ORG" = "mm" ]; then
            macs2 callpeak -t $OUT_FOLDER/2_BAM/"$filename".clean.bam -c $INPUT_CHIP/*.clean.filterdup.bam --name="$filename".filterdup.nol  --outdir $OUT_FOLDER/4_Called_Peaks/ --format=BAM -B -g mm --nomodel --extsize 250 --nolambda --broad --broad-cutoff 0.1 -m 250 500
            rm $OUT_FOLDER/4_Called_Peaks/"$filename".filterdup.nol_control*.bdg 
        else
            if [ "$ORG" = "hg" ]; then
                macs2 callpeak -t $OUT_FOLDER/2_BAM/"$filename".clean.bam -c $INPUT_CHIP/*.clean.filterdup.bam --name="$filename".filterdup.nol  --outdir $OUT_FOLDER/4_Called_Peaks/ --format=BAM -B -g hs --nomodel --extsize 250 --nolambda --broad --broad-cutoff 0.1 -m 250 500
                rm $OUT_FOLDER/4_Called_Peaks/"$filename".filterdup.nol_control*.bdg 
            fi
        fi
    fi
