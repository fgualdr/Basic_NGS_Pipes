...
# -------- Remove multi mappers ----------- ## can use -F 256 to exclude secondary aligmnet or the XS:i flag bowtie2 uses
...
if [ "$CALL_PEAKS" = "YES" ]; then
    ...
    if [ "$INPUT_CHIP" = "NA" ]; then
        echo NO INPUT
        if [ "$ORG" = "mm" ]; then
            macs2 callpeak -t $OUT_FOLDER/2_BAM/"$filename".clean.bam  --name="$filename".nol --outdir $OUT_FOLDER/4_Called_Peaks/ --format=BAMPE -B -g mm --call-summits
        else
            if [ "$ORG" = "hg" ]; then
                macs2 callpeak -t $OUT_FOLDER/2_BAM/"$filename".clean.bam  --name="$filename".nol --outdir $OUT_FOLDER/4_Called_Peaks/ --format=BAMPE -B -g hs --call-summits
            fi
        fi
    else
        if [ "$ORG" = "mm" ]; then
            macs2 callpeak -t $OUT_FOLDER/2_BAM/"$filename".clean.bam  -c $INPUT_CHIP --name="$filename".nol --outdir $OUT_FOLDER/4_Called_Peaks/ --format=BAMPE -B -g mm --call-summits
            rm $OUT_FOLDER/4_Called_Peaks/"$filename".nol_control*.bdg
        else
            if [ "$ORG" = "hg" ]; then
                macs2 callpeak -t $OUT_FOLDER/2_BAM/"$filename".clean.bam  -c $INPUT_CHIP --name="$filename".nol --outdir $OUT_FOLDER/4_Called_Peaks/ --format=BAMPE -B -g hs --call-summits
                 rm $OUT_FOLDER/4_Called_Peaks/"$filename".nol_control*.bdg
            fi
        fi
    fi
