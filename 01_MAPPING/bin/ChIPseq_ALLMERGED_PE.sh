...
if [ "$INPUT_CHIP" = NA ]; then
    echo NO INPUT
    macs2 callpeak -t $OUT_FOLDER/2_BAM/*.clean.bam  --name=ALL_MERGED.nol --outdir $OUT_FOLDER/4_Called_Peaks/ --format=BAMPE -B -g mm --extsize 250 --keep-dup 'all' --call-summits
    ...
    macs2 callpeak -t $OUT_FOLDER/2_BAM/*.clean.bam  -c /hpcnfs/data/GN2/fgualdrini/UsefullData/INPUT_BMDM/INPUT_BMDM.clean.bam --name=ALL_MERGED.nol --outdir $OUT_FOLDER/4_Called_Peaks/ --format=BAMPE -B -g mm --extsize 250 --keep-dup 'all' --call-summits
