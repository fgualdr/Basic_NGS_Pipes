...
bamCoverage -p 20 -bs 1 --scaleFactor $scale_f --filterRNAstrand forward -b $OUT_FOLDER/CLEAN_BAM/"$filename".clean.bam -o $OUT_FOLDER/BIGWIG/"$filename".reverse.scale_depth.bw 
bamCoverage -p 20 -bs 1  --scaleFactor $scale_f --filterRNAstrand reverse -b $OUT_FOLDER/CLEAN_BAM/"$filename".clean.bam -o $OUT_FOLDER/BIGWIG/"$filename".forward.scale_depth.bw 
