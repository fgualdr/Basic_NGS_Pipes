...
if [ "$SEQ_MODE" = "SE" ]; then
    ...
    do
        ...
        echo $filename
        #Adapter Trimming
        bbduk.sh -Xmx1g threads=7 in=$f out=$SAMP_PATH/"$filename".clean.fastq ref=$resource/adapters.fa maq=10 ftm=5 ktrim=r k=28 mink=13 hdist=1 tbo tpe
        rm $f
        ...
        if [ "$SEQ_MODE" = "PE" ]; then
            ...
            do
                ...
                echo $filename
                #Adapter Trimming
                bbduk.sh -Xmx1g threads=7 in1=$SAMP_PATH/"$filename"_R1.fastq in2=$SAMP_PATH/"$filename"_R2.fastq out1=$SAMP_PATH/"$filename".clean.R1.fastq out2=$SAMP_PATH/"$filename".clean.R2.fastq ref=$resource/adapters.fa maq=10 ftm=5 ktrim=r k=28 mink=13 hdist=1 tbo tpe
